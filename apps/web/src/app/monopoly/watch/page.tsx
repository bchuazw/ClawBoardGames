...
function WatchPage() {
  ...
  useEffect(() => {
    ...
    if (gid) {
      router.replace(`/monopoly/watch/lobby/${gid}`);
      return;
    }
    if (gm) setGmUrl(gm);
  }, [searchParams, router]);
    ...
    <div className="watch-layout">
      ...
      <div className="watch-center">
        ...
        <div className="watch-topbar-inner" style={{
          ...
          <button onClick={connected ? () => disconnect() : () => { if (gameId) router.push(`/monopoly/watch/lobby/${gameId}`); else connect(); }}
        ...
        {!gameId && (
          <div style={{
            ...
            <Link href="/monopoly" style={{ position: 'absolute', top: 28, left: 28, display: 'inline-flex', alignItems: 'center', gap: 8, fontSize: 14, fontWeight: 600, color: '#fff', textDecoration: 'none', padding: '12px 20px', borderRadius: 10, background: '#CC5500', border: '1px solid rgba(204,85,0,0.5)', boxShadow: '0 4px 14px rgba(204,85,0,0.25)', transition: 'transform 0.2s, box-shadow 0.2s' }}>
            ...
            <div style={{
              ...
              <p style={{ marginTop: 28, fontSize: 13, color: 'var(--text-muted-soft)', textAlign: 'center' }}>
                Or open <code style={{ background: 'rgba(255,255,255,0.06)', padding: '4px 10px', borderRadius: 6, fontFamily: 'var(--font-mono)', fontSize: 12 }}>/monopoly/watch/lobby/5</code> to jump to game 5
              </p>
            </div>
          </div>
        )}
